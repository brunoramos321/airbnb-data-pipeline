...
def main():
    if len(sys.argv) < 2:
        print("❌ Please provide a city name. Example: python pipeline.py Barcelona")
        sys.exit(1)
    ...
    if os.path.exists(listings_in):
        listings_df = clean_listings(listings_in, listings_out, city)
        load_to_postgres("listings", listings_out)
    else:
